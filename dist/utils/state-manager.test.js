// src/utils/state-manager.test.ts
...
// src/utils/state-manager.test.ts
var TEST_DIR = join2(tmpdir(), "hermes-checkpoint-test");
var CHECKPOINT_FILE = join2(TEST_DIR, "state.json");
function noopLock(_path, fn) {
  return fn();
}
describe("createHermesStateManager", () => {
  beforeEach(() => {
  ...
  test("returns empty checkpoint when file does not exist", async () => {
    const mgr = createHermesStateManager({ filePath: CHECKPOINT_FILE, withFileLock: noopLock });
    const state = await mgr.read();
    expect(state.lastSessionStartedAt).toBeNull();
    expect(state.lastMessageId).toBe(0);
    expect(state.messageIndices).toEqual({});
  });
  test("writes and reads checkpoint", async () => {
    const mgr = createHermesStateManager({ filePath: CHECKPOINT_FILE, withFileLock: noopLock });
    await mgr.write({
      lastSessionStartedAt: 1776436892.92733,
      lastMessageId: 42,
      messageIndices: { "20260417_164106_528354": 5, "20260417_164515_5ccd29": 3 }
    });
    const state = await mgr.read();
    expect(state.lastSessionStartedAt).toBe(1776436892.92733);
    expect(state.lastMessageId).toBe(42);
    expect(state.messageIndices).toEqual({
      "20260417_164106_528354": 5,
      "20260417_164515_5ccd29": 3
    });
  });
  test("overwrites previous checkpoint", async () => {
    const mgr = createHermesStateManager({ filePath: CHECKPOINT_FILE, withFileLock: noopLock });
    await mgr.write({
      lastSessionStartedAt: 1776436892,
      lastMessageId: 10,
      messageIndices: { "20260417_164106_528354": 2 }
    });
    await mgr.write({
      lastSessionStartedAt: 1776444092,
      lastMessageId: 50,
      messageIndices: { "20260417_164106_528354": 8, "20260417_184000_abc123": 1 }
    });
    const state = await mgr.read();
    expect(state.lastMessageId).toBe(50);
    expect(state.messageIndices["20260417_164106_528354"]).toBe(8);
    expect(state.messageIndices["20260417_184000_abc123"]).toBe(1);
  });
  test("creates parent directory if missing", async () => {
    const nested = join2(TEST_DIR, "deep", "nested", "state.json");
    const mgr = createHermesStateManager({ filePath: nested, withFileLock: noopLock });
    await mgr.write({
      lastSessionStartedAt: null,
      lastMessageId: 1,
      messageIndices: {}
  ...
  test("handles corrupted file gracefully", async () => {
    writeFileSync(CHECKPOINT_FILE, "not json at all");
    const mgr = createHermesStateManager({ filePath: CHECKPOINT_FILE, withFileLock: noopLock });
    const state = await mgr.read();
    expect(state.lastSessionStartedAt).toBeNull();
    expect(state.lastMessageId).toBe(0);
