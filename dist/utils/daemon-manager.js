...
async function spawnDaemon() {
  await ensureDirectory(HERMES_ZEST_HOME);
  ...
  const daemonEntry = join3(currentDir, "..", "daemon", "index.js");
  const errFd = openSync(DAEMON_LOG_FILE, "a");
  ...
  child.unref();
  if (child.pid == null) {
    captureException(new Error("Daemon spawn returned no PID"), DAEMON_START_FAILED, "daemon-manager", { ...buildDaemonProperties({ operation: "start" }) });
    return { status: "spawn-failed" };
  }
  await writeFile2(DAEMON_PID_FILE, String(child.pid), "utf-8");
  await new Promise((resolve) => setTimeout(resolve, 300));
  if (!isProcessAlive(child.pid)) {
    captureException(new Error(`Daemon crashed on start (PID: ${child.pid})`), DAEMON_START_FAILED, "daemon-manager", { ...buildDaemonProperties({ operation: "start" }), pid: child.pid });
    try {
      await unlink2(DAEMON_PID_FILE);
    } catch {}
    return { status: "crashed-on-start", pid: child.pid };
  }
  logger2.info("daemon_spawned", { pid: child.pid });
