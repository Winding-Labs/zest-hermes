...
function mapPollSessionToAuthSession(pollSession) {
  ...
  if (pollSession.refresh_token_expires_at) {
    refreshTokenExpiresAt = pollSession.refresh_token_expires_at * 1000;
  } else if (pollSession.refresh_token_expires_in) {
    refreshTokenExpiresAt = now + pollSession.refresh_token_expires_in * 1000;
  }
  return {
...
async function trackPluginInstalled(userId, version2) {
  try {
    ...
    logger.debug("Hermes plugin installed event tracked", { userId });
  } catch (error) {
    logger.debug("Failed to track plugin installed event", error);
  }
}
...
async function stopDaemon() {
  ...
  try {
    ...
    logger.error("daemon_sigterm_failed", { pid, error: String(error) });
  ...
  for (let i = 0;i < 20; i++) {
    if (!isProcessAlive(pid)) {
      ...
      logger.info("daemon_stopped", { pid });
  ...
  try {
    ...
    logger.warn("daemon_force_killed", { pid });
...
`);
    ...
    logger.info("auth_session_saved");
    ...
    if (onDemand) {
      ...
      logger.warn("auth_post_login_skip no_supabase_client");
    }
    const finalSession = await loadSession() ?? session;
    return finalSession;
  } catch (error) {
...
async function initiateDeviceCode() {
  try {
    ...
    logger.info("auth_device_code_initiated");
    console.log(`
...
`);
    ...
    logger.error("auth_initiate_failed", error);
    if (error instanceof Error) {
...
async function openBrowser(deviceCodeData) {
  try {
    ...
    logger.warn("auth_browser_open_failed", error);
  }
}
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (data.status === "authorized" && data.session) {
        logger.info("auth_poll_authorized");
      ...
      logger.warn("auth_poll_attempt_error", error);
      await new Promise((resolve) => setTimeout(resolve, deviceCodeData.interval * 1000));
      continue;
    }
  }
...
async function updateProfileMetadata(supabase, userId) {
  try {
    ...
    const { wasAlreadyInstalled } = await updateExtensionMetadata(supabase, userId, "hermes", version4, { logger });
    if (!wasAlreadyInstalled) {
      await trackPluginInstalled(userId, version4);
    }
    logger.info("auth_profile_metadata_updated");
  } catch (error) {
    logger.warn("auth_profile_metadata_warn", error);
  }
}
async function selectDefaultWorkspace(supabase, session) {
  try {
    const workspaces = await fetchUserWorkspaces(supabase, session.userId, { logger });
    const result = resolveWorkspace(session, workspaces);
    if (!result) {
      logger.warn("auth_workspace_skip no_workspaces");
      return;
    }
    await saveSession(result.session);
    logger.info(`auth_workspace_${result.resolution}`, {
      name: result.workspace.name,
      total: result.totalWorkspaces
    });
  } catch (error) {
    logger.warn("auth_workspace_warn", error);
  }
}
