...
function computeRefreshTokenExpiresAt(expiresAt, expiresIn) {
  if (expiresAt)
...
// ../../packages/plugin-common/src/auth/device-code-requests.ts
async function fetchDeviceCode({
  webAppUrl,
  clientId
}) {
  const response = await fetch(`${webAppUrl}/api/auth/device/initiate`, {
    method: "POST",
...
async function fetchDevicePoll({
  webAppUrl,
  authCode
}) {
  const response = await fetch(`${webAppUrl}/api/auth/device/poll`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function isAuthDeniedError(error) {
  return error instanceof Error && error.message.includes("denied");
}
function isAuthExpiredError(error) {
  return error instanceof Error && error.message.includes("expired");
}
function mapPollSessionToAuthSession(pollSession) {
  return {
    accessToken: pollSession.access_token,
    refreshToken: pollSession.refresh_token,
    refreshTokenExpiresAt: computeRefreshTokenExpiresAt(pollSession.refresh_token_expires_at, pollSession.refresh_token_expires_in),
    userId: pollSession.user.id,
    email: pollSession.user.email || "",
    workspaceId: pollSession.workspace_id
  };
}

// ../../packages/plugin-common/src/auth/workspace.ts
function resolveWorkspace(session, workspaces) {
  if (workspaces.length === 0) {
    return null;
  }
  if (session.workspaceId) {
    const matched = workspaces.find((w) => w.id === session.workspaceId);
    if (matched) {
      return {
        session: { ...session, workspaceName: matched.name },
        workspace: matched,
        resolution: "matched",
  ...
  const sorted = [...workspaces].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
  const fallback = sorted[0];
  return {
    session: { ...session, workspaceId: fallback.id, workspaceName: fallback.name },
    workspace: fallback,
    resolution: "fallback",
    totalWorkspaces: workspaces.length
  };
...
async function fetchWorkspaceName(supabase, workspaceId) {
...
async function trackPluginInstalled(userId, version2) {
  try {
    const client = await getAnalyticsClient();
    if (!client)
      return;
    ...
    logger2.debug("Hermes plugin installed event tracked", { userId });
  } catch (error) {
    logger2.debug("Failed to track plugin installed event", error);
  }
}
...
async function readPidFile() {
  try {
    const content = await readFile2(DAEMON_PID_FILE, "utf-8");
...
async function stopDaemon() {
  ...
  if (!isProcessAlive(pid)) {
    ...
    return { status: "stale-pidfile-cleared", pid };
  }
  try {
    process.kill(pid, "SIGTERM");
  } catch (error) {
    logger2.error("daemon_sigterm_failed", { pid, error: String(error) });
    return { status: "error", pid };
  }
  for (let i = 0;i < 20; i++) {
    if (!isProcessAlive(pid)) {
      try {
      ...
      logger2.info("daemon_stopped", { pid });
  ...
  try {
    ...
    await unlink2(DAEMON_PID_FILE).catch(() => {});
    logger2.warn("daemon_force_killed", { pid });
    return { status: "force-killed", pid };
  } catch {
...
async function runPostLoginOps(session, workspaceId) {
  const onDemand = await createOnDemandClient2();
  if (onDemand) {
    try {
      await updateProfileMetadata(onDemand.client, session.userId);
      if (workspaceId) {
        const name = await fetchWorkspaceName(onDemand.client, workspaceId);
        if (name) {
          session.workspaceName = name;
          await saveSession(session);
        }
      } else {
        await selectDefaultWorkspace(onDemand.client, session);
      }
    } finally {
      await onDemand.dispose();
    }
  } else {
    logger2.warn("post_login_skip no_supabase_client");
  }
  return await loadSession() ?? session;
}
async function startAuthFlow() {
  try {
    logger2.info("auth_start");
    const deviceCodeData = await initiateDeviceCode();
    await openBrowser(deviceCodeData);
    console.log(`⏳ Waiting for authorization...
`);
    const session = await pollForAuthorization(deviceCodeData);
    await saveSession(session);
    logger2.info("auth_session_saved");
    return await runPostLoginOps(session);
  } catch (error) {
    logger2.error("auth_failed", error);
    throw error;
  }
}
async function startAgentAuthFlow(agentId, provisioningKey, workspaceId) {
  try {
    logger2.info("agent_auth_start", { agentId });
    ...
    await saveSession(session);
    logger2.info("agent_auth_session_saved");
    return await runPostLoginOps(session, workspaceId);
  } catch (error) {
    logger2.error("agent_auth_failed", error);
    if (error instanceof Error) {
      await captureException(error, AUTH_AGENT_PROVISIONING_FAILED, "authentication", {
        ...buildAuthProperties({ authMethod: "agent_provisioning" }),
        agentId
      });
    }
    throw error;
...
async function initiateDeviceCode() {
  try {
    const data = await fetchDeviceCode({ webAppUrl: WEB_APP_URL, clientId: CLIENT_ID });
    logger2.info("auth_device_code_initiated");
    console.log(`
\uD83D\uDD10 Enter code in browser: ${data.deviceCode}`);
    console.log(`\uD83D\uDCF1 URL: ${data.verificationUrl}
`);
    return data;
  } catch (error) {
    logger2.error("auth_initiate_failed", error);
    if (error instanceof Error) {
      await captureException(error, AUTH_DEVICE_CODE_INITIATION_FAILED, "authentication", {
...
async function openBrowser(deviceCodeData) {
  try {
    await open_default(deviceCodeData.verificationUrl, { wait: false });
  } catch (error) {
    logger2.warn("auth_browser_open_failed", error);
  }
}
async function pollForAuthorization(deviceCodeData) {
  ...
  while (attempts < maxAttempts) {
    attempts++;
    try {
      const data = await fetchDevicePoll({
        webAppUrl: WEB_APP_URL,
        authCode: deviceCodeData.authCode
      });
      if (data.status === "authorized" && data.session) {
        logger2.info("auth_poll_authorized");
        console.log("✅ Authorization successful!");
        return mapPollSessionToAuthSession(data.session);
      }
      ...
      if (isAuthDeniedError(error)) {
        ...
        await captureException(deniedError, AUTH_DEVICE_CODE_POLLING_FAILED, "authentication", {
          ...buildAuthProperties({ authMethod: "device_code" }),
          reason: "denied"
        });
        throw deniedError;
      ...
      if (isAuthExpiredError(error)) {
        ...
        await captureException(expiredError, AUTH_DEVICE_CODE_POLLING_FAILED, "authentication", {
          ...buildAuthProperties({ authMethod: "device_code" }),
          reason: "expired"
        });
        throw expiredError;
      }
      logger2.warn("auth_poll_attempt_error", error);
      await new Promise((resolve) => setTimeout(resolve, deviceCodeData.interval * 1000));
      continue;
    }
  }
  const timeoutError = new Error("Authorization timeout - no response received");
  await captureException(timeoutError, AUTH_DEVICE_CODE_POLLING_FAILED, "authentication", {
    ...buildAuthProperties({ authMethod: "device_code" }),
    reason: "timeout",
...
async function updateProfileMetadata(supabase, userId) {
  try {
    ...
    const { wasAlreadyInstalled } = await updateExtensionMetadata(supabase, userId, "hermes", version4, { logger: logger2 });
    if (!wasAlreadyInstalled) {
      await trackPluginInstalled(userId, version4);
    }
    logger2.info("auth_profile_metadata_updated");
  } catch (error) {
    logger2.warn("auth_profile_metadata_warn", error);
  }
}
async function selectDefaultWorkspace(supabase, session) {
  try {
    const workspaces = await fetchUserWorkspaces(supabase, session.userId, { logger: logger2 });
    const result = resolveWorkspace(session, workspaces);
    if (!result) {
      logger2.warn("auth_workspace_skip no_workspaces");
      return;
    }
    await saveSession(result.session);
    logger2.info(`auth_workspace_${result.resolution}`, {
      name: result.workspace.name,
      total: result.totalWorkspaces
    });
  } catch (error) {
    logger2.warn("auth_workspace_warn", error);
  }
}
async function logout() {
  try {
    await stopDaemon();
    await clearSession();
    logger2.info("auth_logout");
  } catch (error) {
    logger2.error("auth_logout_failed", error);
    throw error;
  }
}
export {
