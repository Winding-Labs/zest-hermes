...
async function main() {
  try {
    ...
    if (!session) {
      console.log("❌ Not authenticated - use /zest_login first");
      return;
    }
    if (!session.workspaceId) {
    ...
    const result = await withSupabaseClient(async (supabase) => {
      const teamAndProfile = await fetchUserTeamAndProfile(supabase, userId, workspaceId, {
        logger: logger2,
        onError: (error2, { endpoint }) => captureException(error2, API_STANDUP_TEAM_FETCH_FAILED, "commands/standup", {
      ...
      const promptId = await fetchDefaultStandupPromptId(supabase, workspaceId, {
        logger: logger2,
        onError: (error2, { endpoint }) => captureException(error2, API_STANDUP_PROMPT_FETCH_FAILED, "commands/standup", {
          ...buildApiProperties({ endpoint })
        })
      });
      if (!promptId) {
      ...
      if (eligibility && eligibility.reason !== "user_generated_today") {
        const totalNewData = eligibility.new_sessions_count + eligibility.new_messages_count + eligibility.new_events_count;
        if (totalNewData === 0) {
          console.log("ℹ️  No new sessions found for today - nothing to analyze");
          return;
        }
      }
      ...
      if (error) {
        ...
        logger2.error("Edge function error", err);
        captureException(err, API_STANDUP_GENERATION_FAILED, "commands/standup", {
          ...buildApiProperties({
            endpoint: "analyze-sessions",
            responseMessage: err.message
          })
        });
        console.log(`❌ Failed to generate standup: ${err.message}`);
        return;
      }
