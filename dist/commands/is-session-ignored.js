...
async function isSessionIgnored(sessionId) {
  const sessions = await loadIgnoredSessions();
  return sessions.has(sessionId);
}

// src/commands/is-session-ignored.ts
async function main() {
  try {
    ...
    const ignored = await isSessionIgnored(sessionId);
    if (ignored) {
      console.log("\uD83D\uDEAB This session is ignored (not being tracked)");
      console.log(`   id: ${sessionId}`);
      console.log("\uD83D\uDCA1 Use /zest_unignore to re-enable tracking");
    } else {
      console.log("✓ This session is being tracked");
      console.log(`   id: ${sessionId}`);
    }
  } catch (error) {
    logger.error("Failed to check session ignore status", error);
    console.error("❌ Failed to check: " + (error instanceof Error ? error.message : "Unknown error"));
    process.exit(1);
  }
}
main();
