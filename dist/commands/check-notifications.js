...
// src/commands/check-notifications.ts
...
// src/commands/check-notifications.ts
function getUpdateNotification(cache) {
  const { updateAvailable, currentVersion, latestVersion } = cache.versionCheck;
  if (!updateAvailable)
    return null;
  return `Zest update available (v${currentVersion} -> v${latestVersion}). Run: hermes plugins update zest-hermes`;
}
function getStandupNotification(cache) {
  const { message, expiresAt } = cache.standupNotification;
...
async function main() {
  try {
    const notification = await withFileLock(STATUS_CACHE_FILE, async () => {
      const cache = readStatusCache();
      const updateMessage = getUpdateNotification(cache);
      if (updateMessage) {
        cache.versionCheck.updateAvailable = false;
        cache.versionCheck.checkedAt = 0;
        await writeFile3(STATUS_CACHE_FILE, JSON.stringify(cache, null, 2), "utf-8");
        return updateMessage;
      }
      const standupMessage = getStandupNotification(cache);
      if (standupMessage) {
        cache.standupNotification.message = null;
        cache.standupNotification.expiresAt = null;
        await writeFile3(STATUS_CACHE_FILE, JSON.stringify(cache, null, 2), "utf-8");
    ...
    logger2.error("Failed to check notifications", error);
    console.log(JSON.stringify({ notification: null, error: "Failed to read notifications" }));
  }
}
main();
