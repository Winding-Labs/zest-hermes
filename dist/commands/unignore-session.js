...
async function removeIgnoredSession(sessionId) {
...
// src/commands/unignore-session.ts
async function main() {
  try {
    ...
    const removed = await removeIgnoredSession(sessionId);
    if (!removed) {
      console.log("✓ This session is not in the ignore list");
      console.log(`   id: ${sessionId}`);
      return;
    }
    console.log("✅ Session unignored");
    console.log(`   id: ${sessionId}`);
    console.log("");
    console.log("\uD83D\uDCCA This session will be synced and included in standups again");
  } catch (error) {
    logger.error("Failed to unignore session", error);
    console.error("❌ Failed to unignore session: " + (error instanceof Error ? error.message : "Unknown error"));
    process.exit(1);
  }
}
main();
