...
// src/commands/list-ignored-sessions.ts
async function main() {
  try {
    const sessions = await loadIgnoredSessions();
    if (sessions.size === 0) {
      console.log("No ignored sessions");
      return;
    }
    console.log(`Ignored sessions (${sessions.size}):`);
    for (const id of sessions) {
      console.log(`  - ${id}`);
    }
  } catch (error) {
    logger.error("Failed to list ignored sessions", error);
    console.error("❌ Failed to list ignored sessions: " + (error instanceof Error ? error.message : "Unknown error"));
    process.exit(1);
  }
}
main();
