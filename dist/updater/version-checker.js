...
async function checkForHermesUpdate() {
  ...
  if (cacheWithinTtl && cacheMatchesRunningVersion) {
    logger.debug("version_check_cached", { checkedAt });
    return {
  ...
  if (currentVersion === "unknown") {
    logger.warn("version_check_skip", { reason: "current_version=unknown" });
    return {
      updateAvailable: false,
      currentVersion: "unknown",
      latestVersion: "unknown",
  ...
  logger.info("version_check_start", { currentVersion });
  let latestVersion;
  try {
    latestVersion = await fetchLatestVersion();
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.warn("version_check_fetch_failed", { error: errorMessage });
    return {
      updateAvailable: false,
      currentVersion,
      latestVersion: "unknown",
  ...
  logger.info("version_check_done", { currentVersion, latestVersion, updateAvailable });
