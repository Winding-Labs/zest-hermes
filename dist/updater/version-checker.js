...
// ../../packages/plugin-common/src/supabase/anon-client.ts
var anonClient = null;
var cachedUrl = "";
var cachedKey = "";
function createAnonClient(supabaseUrl, supabaseAnonKey) {
  if (!supabaseUrl || !supabaseAnonKey)
    return null;
  if (!anonClient || supabaseUrl !== cachedUrl || supabaseAnonKey !== cachedKey) {
...
function buildStandardProperties(version3) {
  return {
...
async function persistRefreshedSession(options, supabaseSession) {
  const { sessionManager, logger, onCaptureException } = options;
  const withFileLock = resolveFileLock(options.withFileLock);
  const sessionFilePath = sessionManager.getSessionFilePath();
  try {
...
async function setClientSession(options, client, session) {
  const { sessionManager, logger, onCapture, onCaptureException } = options;
  const { error } = await client.auth.setSession({
    access_token: session.accessToken,
    refresh_token: session.refreshToken
  });
  if (error) {
    logger?.error(`Failed to set Supabase session: ${error.message}`);
    ...
    if (isAlreadyUsed) {
      ...
      await sessionManager.clearSessionIfStale(session.refreshToken);
      throw error;
    }
...
class PersistentClient {
  ...
  async getClient() {
    const { logger, onCaptureException } = this.options;
    try {
      ...
      if (!this.client) {
        ...
        const { data } = this.client.auth.onAuthStateChange(async (event, supabaseSession) => {
          ...
          if (event === "TOKEN_REFRESHED" && supabaseSession) {
            ...
            logger?.warn("Received SIGNED_OUT event from Supabase, destroying persistent client");
            await this.destroy();
          }
        });
...
function createStatusCacheManager(config) {
  ...
  function readStatusCache() {
    try {
      ...
      if (error.code === "ENOENT") {
        ...
        logger3?.warn("Failed to read status cache file, using defaults", error);
      }
  ...
  async function writeVersionCheck(check) {
    try {
      ...
      logger3?.error("Failed to write version check to status cache", error);
    }
  }
  async function writeLastUpdate(update) {
    try {
      ...
      logger3?.error("Failed to write last update to status cache", error);
    }
  }
  async function writeSyncStatus(status) {
    try {
      ...
      logger3?.error("Failed to write sync status to status cache", error);
    }
  }
  async function clearSyncError() {
    try {
      ...
      logger3?.error("Failed to clear sync error in status cache", error);
    }
  }
  async function writeDevModeActive() {
    try {
      ...
      logger3?.error("Failed to write dev mode to status cache", error);
    }
  }
  async function writeStandupNotification(message, options) {
    try {
      ...
      logger3?.error("Failed to write standup notification to status cache", error);
    }
  }
  function hasActiveStandupNotification() {
    try {
      ...
      logger3?.warn("Failed to check for active standup notification", error);
      return false;
    }
  }
  function shouldShowFirstDataReady() {
    try {
      ...
      logger3?.warn("Failed to check first data ready throttle", error);
      return true;
    }
  }
  async function writeDaemonWarmingUp(durationMs) {
    try {
      ...
      logger3?.error("Failed to write daemon warmup to status cache", error);
    }
  }
  function shouldShowStandupRefreshed() {
    try {
      ...
      logger3?.warn("Failed to check standup refreshed throttle", error);
      return true;
    }
  }
  return {
...
async function fetchLatestVersion() {
  ...
  if (!isStrictVersion(data.latest_version)) {
    throw new Error(`extension_versions: invalid version: ${data.latest_version}`);
  }
...
async function checkForHermesUpdate() {
  ...
  if (currentVersion === "unknown") {
    logger2.warn("version_check_skip", { reason: "current_version=unknown" });
    return {
      updateAvailable: false,
      currentVersion: "unknown",
      latestVersion: "unknown",
  ...
  logger2.info("version_check_start", { currentVersion });
  let latestVersion;
  try {
    latestVersion = await fetchLatestVersion();
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger2.warn("version_check_fetch_failed", { error: errorMessage });
    return {
      updateAvailable: false,
      currentVersion,
      latestVersion: "unknown",
  ...
  const updateAvailable = compareVersions(currentVersion, latestVersion) === "newer";
  const versionCheck = {
    updateAvailable,
    currentVersion,
    latestVersion,
  ...
  await writeVersionCheck(versionCheck);
  logger2.info("version_check_done", { currentVersion, latestVersion, updateAvailable });
  return { updateAvailable, currentVersion, latestVersion };
}
export {
  checkForHermesUpdate
};
